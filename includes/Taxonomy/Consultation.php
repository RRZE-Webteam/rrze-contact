...
use RRZE\Contact\Capabilities;
use RRZE\Contact\Functions;
use function RRZE\Contact\Config\isAllowedSearchForGuest;
// use RRZE\Contact\Carbon;
...
class Consultation
{
    ...
    {
        $labels = [
            'name'                      => _x('Bookings', 'Post type general name', 'rrze-contact'),
            'singular_name'             => _x('Booking', 'Post type singular name', 'rrze-contact'),
            'menu_name'                 => _x('Bookings', 'Admin Menu text', 'rrze-contact'),
            'name_admin_bar'            => _x('Booking', 'Add New on Toolbar', 'rrze-contact'),
            'add_new'                   => __('Add New', 'rrze-contact'),
            'add_new_item'              => __('Add New Booking', 'rrze-contact'),
            'new_item'                  => __('New Booking', 'rrze-contact'),
            'edit_item'                 => __('Edit Booking', 'rrze-contact'),
            'view_item'                 => __('View Booking', 'rrze-contact'),
            'all_items'                 => __('All Bookings', 'rrze-contact'),
            'search_items'              => __('Search Bookings', 'rrze-contact'),
            'not_found'                 => __('No Bookings found.', 'rrze-contact'),
            'not_found_in_trash'        => __('No Bookings found in Trash.', 'rrze-contact'),
            'archives'                  => _x('Booking archives', 'The post type archive label used in nav menus. Default “Post Archives”. Added in 4.4', 'rrze-contact'),
            'filter_items_list'         => _x('Filter Bookings list', 'Screen reader text for the filter links heading on the post type listing screen. Default “Filter posts list”/”Filter pages list”. Added in 4.4', 'rrze-contact'),
            'items_list_navigation'     => _x('Bookings list navigation', 'Screen reader text for the pagination heading on the post type listing screen. Default “Posts list navigation”/”Pages list navigation”. Added in 4.4', 'rrze-contact'),
            'items_list'                => _x('Bookings list', 'Screen reader text for the items list heading on the post type listing screen. Default “Posts list”/”Pages list”. Added in 4.4', 'rrze-contact'),
        ];

        $args = [
            'label'                     => __('Booking', 'rrze-contact'),
            'description'               => __('Add and edit Booking informations', 'rrze-contact'),
            'labels'                    => $labels,
    ...
    public function addBookingColumns($columns)
    {
        $columns = array();
        $columns['cb'] = true;
        $columns['bookingdate'] = __('Date', 'rrze-contact');
        $columns['bookingstart'] = __('Time', 'rrze-contact');
        $columns['room'] = __('Room', 'rrze-contact');
        $columns['seat'] = __('Seat', 'rrze-contact');
        $columns['name'] = __('Name', 'rrze-contact');
        $columns['email'] = __('Email', 'rrze-contact');
        if (CORONA_MODE && current_user_can('read_customer_phone')) {
            $columns['phone'] = __('Phone', 'rrze-contact');
        }
        $columns['status'] = __('Status', 'rrze-contact');
        return $columns;
    }
    ...
    function getBookingValue($column, $post_id)
    {
        ...
        switch ($column) {
            ...
            case 'status':
                ...
                $bookingDate = '<span class="booking_date">' . __('Booked on', 'rrze-contact') . ' ' . $booking['booking_date'] . '</span>';
                $archive = ($end < $now);
                $publish = ($booking['post_status'] == 'publish');
                $bookingMode = get_post_meta($booking['room'], 'rrze-contact-room-bookingmode', true);

                if ($publish && $archive) {
                    switch ($status) {
                        case 'cancelled':
                            $button = '<span class="delete">' . _x('Cancelled', 'Booking', 'rrze-contact') . '</span>';
                            break;
                        case 'booked':
                            $button = '<span class="delete">' . _x('Booked', 'Booking', 'rrze-contact') . '</span>';
                            break;
                        case 'confirmed':
                            $button = '<span class="delete">' . _x('Confirmed', 'Booking', 'rrze-contact') . '</span>';
                            break;
                        case 'checked-in':
                            $button = _x('Checked-In', 'Booking', 'rrze-contact');
                            break;
                        case 'checked-out':
                            $button = _x('Checked-Out', 'Booking', 'rrze-contact');
                            break;
                        default:
                            $button = '';
                    }
                    ...
                    if ($status == 'cancelled') {
                        $cancelledButton = '<button class="button button-secondary" disabled>' . _x('Cancelled', 'Booking', 'rrze-contact') . '</button>';
                        // deactivate restore
                        ...
                        //     $booking['id'],
                        //     $_wpnonce,
                        //     _x('Restore', 'Booking', 'rrze-contact')
                        // );
                        ...
                        $cancelButton = sprintf(
                            ...
                            $booking['id'],
                            $_wpnonce,
                            _x('Cancel', 'Booking', 'rrze-contact')
                        );
                        $checkInButton = sprintf(
                            ...
                            $booking['id'],
                            $_wpnonce,
                            _x('Check-In', 'Booking', 'rrze-contact')
                        );
                        $checkoutButton = sprintf(
                            ...
                            $booking['id'],
                            $_wpnonce,
                            _x('Check-Out', 'Booking', 'rrze-contact')
                        );
                        $forceToConfirm = Functions::getBoolValueFromAtt(get_post_meta($booking['room'], 'rrze-contact-room-force-to-confirm', true));
                        ...
                        if ($bookingMode == 'check-only') {
                            switch ($status) {
                                case 'checked-in':
                                    $button = '<button class="button button-primary" disabled>' . _x('Checked-In', 'Booking', 'rrze-contact') . '</button>' . $checkoutButton;
                                    break;
                                case 'checked-out':
                                    $button = '<button class="button button-primary" disabled>' . _x('Checked-Out', 'Booking', 'rrze-contact') . '</button>' . $checkInButton;
                                    break;
                                case 'booked':
                                default:
                            ...
                            $button = _x('Waiting for customer confirmation', 'Booking', 'rrze-contact') . $cancelButton;
                            ...
                            $button = $cancelButton . sprintf(
                                ...
                                $booking['id'],
                                $_wpnonce,
                                _x('Confirm', 'Booking', 'rrze-contact')
                            );
                        }
                    }
                    echo $button, $bookingDate;
    ...
    public function ajaxShowTimeslots()
    {
        ...
        if (isset($availability[$date])) {
            $output .= '<select class="select_timeslot">'
                . '<option value="">' . __('Select timeslot', 'rrze-contact') . '</option>';
            ...
            $output .= __('No timeslots available for this seat/day.', 'rrze-contact');
        }
        $output .= '</div>';
        echo $output;
        wp_die();
    ...
    public function addFilters($post_type)
    {
        ...
        $sAllDates = __('Show all dates', 'rrze-contact');
        $sAllTimeslots = __('Show all time slots', 'rrze-contact');
        $sAllRoomes = __('Show all rooms', 'rrze-contact');
        ...
        foreach ($aBookingIds as $bookingId) {
            // 2. get unique dates
            $bookingStart = get_post_meta($bookingId, 'rrze-contact-booking-start', true);
            $aBookingDates[date("Y-m-d", $bookingStart)] = Functions::dateFormat((int)$bookingStart);

            $bookingEnd = get_post_meta($bookingId, 'rrze-contact-booking-end', true);
            ...
            $seatId = get_post_meta($bookingId, 'rrze-contact-booking-seat', true);
            $roomId = get_post_meta($seatId, 'rrze-contact-seat-room', true);
    ...
    private function getBookingIDsBySeatRoomTitle($sSearch)
    {
        global $wpdb;
        $aBookingIDs = [];
        $aIDs = $wpdb->get_results("SELECT ID FROM $wpdb->posts p, $wpdb->postmeta pm WHERE p.post_type = 'booking' AND p.post_status = 'publish' AND p.id = pm.post_id AND pm.meta_key = 'rrze-contact-booking-seat' AND pm.meta_value IN (
                SELECT ID FROM $wpdb->posts p, $wpdb->postmeta pm WHERE p.post_type = 'seat' AND p.post_status = 'publish' AND p.id = pm.post_id AND (p.post_title LIKE '%" . $sSearch . "%' OR (pm.meta_key = 'rrze-contact-seat-room' AND pm.meta_value IN 
    ...
    private function getBookingIDsByFilter()
    {
        ...
        if ($this->filterRoomIDs) {
            $sql .= " AND ID IN (SELECT post_id FROM $wpdb->postmeta WHERE meta_key = 'rrze-contact-booking-seat' AND meta_value IN (SELECT post_id FROM $wpdb->postmeta WHERE meta_key = 'rrze-contact-seat-room' AND meta_value = $this->filterRoomIDs))";
        }

        if ($this->filterDate) {
            $sql .= " AND ID IN (SELECT post_id FROM $wpdb->postmeta WHERE (meta_key = 'rrze-contact-booking-start' OR meta_key = 'rrze-contact-booking-end') AND DATE_FORMAT(FROM_UNIXTIME(meta_value), '%Y-%m-%d') = '$this->filterDate')";
        }

        if ($this->filterStart) {
            $sql .= " AND ID IN (SELECT post_id FROM $wpdb->postmeta WHERE meta_key = 'rrze-contact-booking-start' AND DATE_FORMAT(FROM_UNIXTIME(meta_value), '%H:%i') = '$this->filterStart')";
        }

        if ($this->filterEnd) {
            $sql .= " AND ID IN (SELECT post_id FROM $wpdb->postmeta WHERE meta_key = 'rrze-contact-booking-end' AND DATE_FORMAT(FROM_UNIXTIME(meta_value), '%H:%i') = '$this->filterEnd')";
        }
    ...
    public function searchBookings($query)
    {
        ...
        switch ($orderby) {
            case 'bookingdate':
                $query->set('meta_key', 'rrze-contact-booking-start');
                $query->set('orderby', 'meta_value_num');
                break;
                // case 'email':
                //     $query->set('meta_key', 'rrze-contact-booking-guest-email');
                //     $query->set('orderby', 'meta_value');
                // break;
                // case 'room':
                // $query->set('meta_key', 'rrze-contact-booking-seat');
                // $query->set('orderby', get_the_title(get_post_meta('meta_value', 'rrze-contact-seat-seat', true)));
                ...
                // break;
                // case 'status':
                //     $query->set('meta_key', 'rrze-contact-booking-status');
